<x-modal name="addCustomField" aside>
    ...
    <form @submit.prevent="submitForm" x-data="addNewField" class="mt-4">
        ...
        <input x-model="name" type="text" id="fieldname" name="fieldname"
        ...
        <div class="mt-4">
            ...
            <div class="flex flex-wrap gap-2.5 mt-4">
                ...
                @foreach($fieldTypes as $type)
                    <label for="type" @click="type = '{{$type['type']}}'"
                           x-bind:class="type === '{{$type['type']}}' ? 'border border-green-500 text-green-500' : 'border border-slate-300' "
                           class="text-sm bg-slate-50  px-1.5 rounded-sm cursor-pointer">{{$type['name']}}
                    </label>
                    <input type="radio" id="type" name="type" value="{{$type['type']}}" class="hidden" x-model="type"/>
                @endforeach
            </div>
        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('addNewField', () => ({
                ...
                submitForm() {
                    // Name validieren
                    // Type muss existieren
                    // als Objekt an Livewire Komponente senden
